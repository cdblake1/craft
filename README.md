...
| **`craft-journal` MCP** | Resume continuity and findings reuse. One-call `journal_resume` orient block, relevance-ranked (BM25, usage- and recency-aware, optional opt-in embeddings) search, self-feeding auto-promoted findings, per-branch/repo/user scopes, finding lifecycle (supersede/retire/update), a curation queue, and typed links. 15 tools. |
