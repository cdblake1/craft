...
| Figma's remote MCP server only accepts allowlisted clients (VS Code, Cursor, Claude Code per the install docs); Copilot CLI may not connect | H | Run the UI/UX agent under a supported host (craft is host-agnostic and supports Claude Code, which has an official Figma plugin), or verify Copilot CLI connectivity / join the client waitlist; the `figma-mcp-spike` phase settles this before anything is built on it |
