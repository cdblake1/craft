...
function _bm25Score(queryTerms, docTokens, stats) {
    ...
    for (const t of queryTerms) {
        ...
        score += idf * (f * (k1 + 1)) / (f + k1 * (1 - b + b * (dl / stats.avgdl)));
    }
    return score;
}
...
function createJournals(store) {
    ...
    function _findingResult(doc, queryTerms, score) {
        const body = doc.body;
    ...
    function findFindings(query, opts) {
        ...
        for (const j of listAll()) {
            ...
            for (const name of _childMd(`${j.key}/findings`)) {
                ...
                if (!body) { continue; }
                docs.push({ repo: j.repo, branch: j.branch, key, body, tokens: _tokenize(body) });
            }
        }
        if (queryTerms.length === 0) { return docs.map(d => _findingResult(d, null, 0)); }
        ...
        const scored = [];
        for (const d of docs) {
            const score = _bm25Score(queryTerms, d.tokens, stats);
            if (score <= 0) { continue; }
            scored.push(_findingResult(d, queryTerms, score));
        }
    ...
    function getFinding(key) {
        ...
        return { key, title: _firstH1Title(body), when_to_read: _extractSection(body, 'When to read this'), body };
    }
    ...
    function createFinding(branch, repo, title, body) {
        const j = findJournal(branch, repo);
        ...
        const existing = _childMd(`${j.key}/findings`);
        ...
        const key = `${j.key}/findings/${prefix}-${slugifyTitle(title)}.md`;
    ...
    function buildFindingsInjection(opts) {
        ...
        return { text, count: results.length, items: results.map(r => ({ key: r.key, score: r.score, repo: r.repo, branch: r.branch, title: r.title })) };
    }
    ...
    return {
        ...
        createFinding, appendStepLog, updateCurrentPlan, buildFindingsInjection,
        slugifyTitle,
    };
}
